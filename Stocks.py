...
def takeinput():
    company = input("Company Abbreviation: ")
    if company == "":
        ...
        takeinput()
    elif not Share(company).get_price():
...
def decision(today, oneday, twoday, threeday, fiveday, oneweek, twohundredavg, fiftyavg):
    ...
    if negative > positive:
        if negative == 3:
            print("This stock looks like it is doing VERY poor! I wowuldn't invest in it.")
            end()
...
def download_daily_ohlc(symbol, start, end):
...
# Start the work flow with greeting
# greeting()
